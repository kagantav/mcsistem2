...
const NEWS = blogs.map((b, i) => ({ slug: b.slug, title: b.title, date: b.date, cat: b.category, img: b.image, excerpt: b.excerpt, featured: i === 0 }));
...
export function NewsGrid() {
  ...
  return (
    <div>
      ...
      {featured && (
        <Link href={`/haberler/${featured.slug}`} className="group grid lg:grid-cols-2 gap-8 lg:gap-10 items-center mb-14 lg:mb-20">
          ...
          <div>
            ...
            <p className="text-[#5a6b82] leading-relaxed mb-5 max-w-xl line-clamp-3">{featured.excerpt}</p>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {rest.map((n, i) => (
          <motion.div key={n.slug} layout initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: 0.5, delay: (i % 3) * 0.08 }}>
            <Link href={`/haberler/${n.slug}`}
              className="group flex flex-col h-full rounded-2xl overflow-hidden bg-white border border-black/[0.07] hover:shadow-2xl hover:-translate-y-1.5 transition-all duration-300">
              ...
              <div className="p-6 flex flex-col flex-1">
                ...
                <h3 className="text-lg font-bold leading-snug mb-3 transition-colors group-hover:text-[#2f6fe0] line-clamp-3" style={{ ...disp, color: NAVY }}>{n.title}</h3>
                <span className="mt-auto inline-flex items-center gap-2 text-sm font-semibold group-hover:gap-3 transition-all" style={{ color: BLUE }}>Devamı →</span>
              </div>
            </Link>
          </motion.div>
        ))}
